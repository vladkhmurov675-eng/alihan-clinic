...
export async function login(
  ...
  if (doctor) {
    ...
    cookieStore.set(
      "session",
      JSON.stringify({ role: "doctor", doctorId: doctor.id }),
      {
        httpOnly: true,
...
export async function sendOTP(phone: string, code: string) {
  await sendWhatsAppMessage(
    phone,
    `Здравствуйте! Ваш код подтверждения: ${code}\nКод действителен 90 секунд.`,
  );
}
...
export async function updateDoctorSettings(data: {
  ...
  const password =
    data.password && !data.password.startsWith("$2")
...
export async function bookAppointment(
  ...
  try {
    ...
    const clinicSetting = await prisma.setting.findUnique({
...
export async function getDoctorAppointments(filters?: {
  ...
  return prisma.appointment.findMany({
    where: {
      ...
      ...(filters?.from &&
        filters.to && { date: { gte: filters.from, lte: filters.to } }),
